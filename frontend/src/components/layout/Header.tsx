...
import { Bell, ChevronDown, Menu, LogOut, User as UserIcon, Sun, Moon, CheckCheck, BellOff, ExternalLink, Settings, Shield, Mail, BadgeCheck } from 'lucide-react'
      ...
      <div className="flex items-center gap-4 sm:gap-6">
        ...
        <div className="relative" ref={profileRef}>
          <button
            ...
            className="flex items-center gap-2.5 p-1 sm:p-1.5 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer group"
          >
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-700 text-white font-extrabold flex items-center justify-center text-sm shadow-md shadow-blue-600/25 group-hover:scale-105 transition-transform">
              {initial}
            </div>
          ...
          {/* Redesigned Enterprise Profile Dropdown Menu */}
          {profileMenuOpen && (
            <div className="absolute right-0 mt-3 w-72 sm:w-80 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200/90 dark:border-slate-800 z-50 overflow-hidden divide-y divide-slate-100 dark:divide-slate-800 animate-in fade-in zoom-in-95 duration-150">
              
              {/* Profile Avatar & Header Banner */}
              <div className="p-5 bg-gradient-to-b from-slate-50 to-white dark:from-slate-800/60 dark:to-slate-900">
                <div className="flex items-center gap-3.5 mb-4">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 text-white font-extrabold flex items-center justify-center text-lg shadow-md shadow-blue-600/30 shrink-0">
                    {initial}
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-md text-[10px] font-extrabold tracking-wider uppercase bg-blue-50 dark:bg-blue-950/70 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-900/60">
                      <Shield size={11} />
                      {displayRole}
                    </span>
                  </div>
                ...
                <div className="space-y-2.5 pt-1 text-xs">
                  <div className="flex flex-col gap-0.5 bg-white dark:bg-slate-800/80 p-2.5 rounded-xl border border-slate-200/80 dark:border-slate-700/60">
                    ...
                    <span className="font-mono font-bold text-slate-800 dark:text-slate-100 text-xs">
                      {user?.employee_id || (user?.id ? `EMP-${String(user.id).padStart(4, '0')}` : '—')}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col gap-0.5 bg-white dark:bg-slate-800/80 p-2.5 rounded-xl border border-slate-200/80 dark:border-slate-700/60">
                    ...
                    <span className="text-slate-700 dark:text-slate-200 font-medium truncate text-xs" title={user?.email}>
                      {user?.email || '—'}
                    </span>
                  </div>
                </div>
              ...
              <div className="p-2 space-y-1">
                <button
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center gap-3 transition-colors cursor-pointer group"
                >
                  <Settings size={16} className="text-slate-500 dark:text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors" />
              ...
              <div className="p-2 bg-slate-50/50 dark:bg-slate-900/50">
                <button
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl text-xs font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/50 flex items-center gap-3 transition-colors cursor-pointer"
                >
                  <LogOut size={16} />
                  <span>Sign Out</span>
                </button>
